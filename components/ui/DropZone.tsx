...
export function DropZone({ onAwaken }: { onAwaken: (data: any) => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-black text-white pointer-events-auto">
      ...
      <div className="flex gap-4 mb-8 border-b border-neutral-800 pb-4">
        <button onClick={() => setActiveTab('upload')} className={activeTab === 'upload' ? 'text-cyan-400 font-bold' : 'text-neutral-500'}>
          Upload Image
        </button>
        <button onClick={() => setActiveTab('web3')} className={activeTab === 'web3' ? 'text-cyan-400 font-bold' : 'text-neutral-500'}>
          NFT Contract
        </button>
      </div>

      {activeTab === 'upload' ? (
        <label className="border-2 border-dashed border-neutral-700 w-96 h-64 flex flex-col items-center justify-center rounded-xl cursor-pointer hover:border-cyan-400 transition-all">
        ...
        <div className="flex flex-col gap-4 w-96">
          <input 
            ...
            className="p-4 bg-neutral-900 rounded-lg border border-neutral-800 focus:border-cyan-400 outline-none text-white"
